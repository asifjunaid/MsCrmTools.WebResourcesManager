...
namespace MscrmTools.WebresourcesManager.AutoResponder
{
    public class ProxyTestController : IDisposable
    {
        ...
        private ConcurrentBag<ProxyResource> proxyResources = new ConcurrentBag<ProxyResource>();
        ...
        private async Task onRequest(object sender, SessionEventArgs e)
        {
            ...
            //if (e.HttpClient.Request.RequestUri.AbsoluteUri.Contains("WebResources/vrp_/CallCenter/EnterpriseMenu"))
            string absoluteUri = e.HttpClient.Request.RequestUri.AbsoluteUri;
            if (proxyResources.Where(x=>absoluteUri.Contains(x.Server) && absoluteUri.Contains(x.UrlPath)).Count()>0)
            {
                var proxyResource = proxyResources.Where(x => absoluteUri.Contains(x.UrlPath)).FirstOrDefault();

                
                byte[] fileData =File.ReadAllBytes(proxyResource.FilePath);
                var header = new HttpHeader("Cache-Control", "no-store");
                
                e.Ok(fileData,new List<HttpHeader>() { header });
                Console.WriteLine($"AUTORESPONDER File with {absoluteUri} served from");
                Console.WriteLine("AUTORESPONDER "+proxyResource.FilePath);

                /*e.Ok(@"<html><head>
                        <meta><style type='text/css'>P { margin: 0; }</style><meta><style type='text/css'>P { margin: 0; }</style><meta></head>
                        <body onfocusout='parent.setEmailRange();' style='overflow-wrap: break-word;'>
                        <b>Complaint Web Application From Proxy</b>
                        </body></html>");
                        */
            }
        ...
        public void UpdateProxyResources(List<ProxyResource> list) {            
            while (!proxyResources.IsEmpty)
            {
                ProxyResource obj;
                proxyResources.TryTake(out obj);
            }
            list.ForEach(x => proxyResources.Add(x));
        }
    }
}
